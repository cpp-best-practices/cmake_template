...
void game_iteration_canvas()
{
  ...
  auto game_iteration = [&](const std::chrono::steady_clock::duration elapsed_time) {
    ...
    for (std::size_t row = 0; row < max_row; ++row) {
      for (std::size_t col = 0; col < bitmap->width(); ++col) { ++(bitmap->at(col, row).R); }
    }

    for (std::size_t row = 0; row < bitmap->height(); ++row) {
      for (std::size_t col = 0; col < max_col; ++col) { ++(bitmap->at(col, row).G); }
    }
    ...
    ++max_row;
    if (max_row >= bitmap->height()) { max_row = 0; }
    ++max_col;
    if (max_col >= bitmap->width()) { max_col = 0; }
  };
